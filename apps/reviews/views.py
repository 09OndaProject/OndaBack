...
class ReviewSummaryView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response('리뷰 요약 정보 반환', ReviewDisplaySerializer(many=True))}
    )
    def get(self, request, meet_id):
        reviews = Review.objects.filter(meet_id=meet_id).order_by('-created_at')[:4]
        avg_rating = Review.objects.filter(meet_id=meet_id).aggregate(avg=Avg('rating'))['avg'] or 0
...
class ReviewListCreateView(generics.ListCreateAPIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response('리뷰 목록 및 평균 평점 반환', ReviewDisplaySerializer(many=True))}
    )
