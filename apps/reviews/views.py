...
class ReviewSummaryView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(
        ...
        tags=["리뷰 API"],
        manual_parameters=[
            openapi.Parameter("meet_id", openapi.IN_PATH, description="모임 ID", type=openapi.TYPE_INTEGER, required=True),
        ],
    ...
    def get(self, request, meet_id):
        ...
        top_reviews = Review.objects.filter(meet=meet).order_by("-rating", "-created_at")[:4]
        summary = Review.objects.filter(meet=meet).aggregate(avg=Avg("rating"), count=Count("id"))
...
class ReviewListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return Review.objects.filter(meet_id=self.kwargs["meet_id"]).order_by("-created_at")
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = ReviewCreateSerializer(data=request.data, context=self.get_serializer_context())
...
class ReviewDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = ReviewDisplaySerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly, IsOwnerOrReadOnlyWithin7Days]

    @swagger_auto_schema(operation_summary="리뷰 상세 조회", tags=["리뷰 API"], responses={200: ReviewDisplaySerializer()})
    def get(self, request, *args, **kwargs):
        return self.retrieve(request, *args, **kwargs)

    @swagger_auto_schema(operation_summary="리뷰 수정", tags=["리뷰 API"], request_body=ReviewCreateSerializer, responses={200: ReviewDisplaySerializer()})
    ...
    @swagger_auto_schema(operation_summary="리뷰 삭제", tags=["리뷰 API"], responses={204: openapi.Response("삭제 성공")})
