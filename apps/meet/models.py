...
class Meet(TimestampModel):
    ...
    area = models.ForeignKey(
        Area, on_delete=models.SET_NULL, null=True, verbose_name="지역"
    )
    digital_level = models.ForeignKey(
        DigitalLevel, on_delete=models.SET_NULL, null=True, verbose_name="디지털난이도"
    )
    category = models.ForeignKey(
        Category, on_delete=models.SET_NULL, null=True, verbose_name="카테고리"
    )
    file = models.OneToOneField(
        File, on_delete=models.SET_NULL, null=True, verbose_name="대표이미지"
    )
    ...
    link = models.URLField(
        max_length=500, null=True, blank=True, verbose_name="오픈채팅방링크"
    )
